...
const getPlayers = async (kpId) => {
  try {
    ...
    // Проверяем что data это объект с плеерами
    ...
    if (!isValidData) {
      console.log('API returned invalid data, using fallback players')
      return getFallbackPlayers(kpId)
    }
    
    return data
  } catch (error) {
    console.error('Error fetching players, using fallback:', error)
    return getFallbackPlayers(kpId)
  }
}
...
const getShikiPlayers = async (shikiId) => {
  try {
    ...
    if (!isValidData) {
      console.log('API returned invalid data, using fallback shiki players')
      return getFallbackShikiPlayers(shikiId)
    }
    
    return data
  } catch (error) {
    console.error('Error fetching shiki players, using fallback:', error)
    return getFallbackShikiPlayers(shikiId)
  }
}
...
const getFallbackShikiPlayers = (shikiId) => {
  return {
    ...
    animego: {
      iframe: `https://animego.org/embed/shikimori/${shikiId}`,
      translate: 'AnimeGo'
    },
    sovetromantica: {
      iframe: `https://sovetromantica.com/embed/shikimori/${shikiId}`,
