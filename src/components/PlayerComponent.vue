...
const fetchPlayers = async () => {
  try {
    ...
    // Проверяем что players это объект с данными
    if (!players || typeof players !== 'object' || Object.keys(players).length === 0) {
      console.error('No players returned, this should not happen with fallback')
      errorMessage.value = 'Не удалось загрузить плееры'
      return
    }
    ...
    console.log('Players internal:', playersInternal.value)
    ...
    if (playersInternal.value.length > 0) {
      ...
      console.log('Selected player:', selectedPlayerInternal.value)
      emit('update:selectedPlayer', selectedPlayerInternal.value)
    } else {
      console.error('playersInternal is empty after mapping')
      errorMessage.value = 'Не удалось загрузить плееры'
    }
  } catch (error) {
...
const openPlayerModal = () => {
  console.log('Opening player modal, players:', playersInternal.value)
  if (playersInternal.value.length === 0) {
    console.error('Cannot open modal: no players available')
    return
  }
